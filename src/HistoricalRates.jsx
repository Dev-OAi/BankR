    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!data || data.length === 0) {
          throw new Error("No historical data found. The GitHub Action may not have run yet.");
        }
        
        const latestEntry = data[data.length - 1];
        setHistory(latestEntry.data);
            ...
            <table className="min-w-full bg-gray-800 rounded-lg shadow-lg">
              <thead>
                <tr className="bg-gray-700">
                  <th className="p-4 text-left cursor-pointer" onClick={() => requestSort('bank_name')}>
                    Bank {getSortIndicator('bank_name')}
                  </th>
                  <th className="p-4 text-left">Account Name</th>
                  <th className="p-4 text-right cursor-pointer" onClick={() => requestSort('apy')}>
                    APY {getSortIndicator('apy')}
                  </th>
                  <th className="p-4 text-right">Min. Deposit</th>
                  {/* REMOVED the Last Updated header */}
                </tr>
              </thead>
              <tbody>
                {sortedHistory.map((rate, index) => (
                  <tr key={`${rate.accountId || index}`} className={`border-t border-gray-700 ${index % 2 === 0 ? 'bg-gray-800' : 'bg-gray-800/50'} hover:bg-gray-700 transition-colors`}>
